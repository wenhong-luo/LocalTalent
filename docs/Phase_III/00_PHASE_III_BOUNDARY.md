...
- `docs/Phase_III/招聘者搜索简历分阶段开发规划.md/html` 已新增为后续专项规划文件，并将 `/company` 的“搜索简历”拆成 6 轮；其中 Prompt 1（边界重签与数据源设计）、Prompt 2（后端受控搜索接口与六个下拉筛选）、Prompt 3（搜索简历列表页高保真 UI）和 Prompt 4（安全简历详情抽屉与举报简历）已完成。当前已登记 `phase3.company_resume_search=false`，默认关闭；新增 `V0026__extend_publish_snapshot_resume_search_fields.sql`、`V0027__create_company_resume_snapshot_report.sql`、company 私有 `GET /api/company/workbench/resume-search`、`GET /api/company/workbench/resume-search/{snapshotId}` 和 `POST /api/company/workbench/resume-search/{snapshotId}/reports`。该能力唯一允许数据源为 `candidate_publish_snapshot` 发布快照，详情只返回安全摘要，举报只保存摘要并写审计/风控任务；仍不得开放公共简历库、联系解锁、真实下载完整简历、手机号/邮箱/微信展示、真实短信微信通知、会员支付或套餐扣点。下一轮如继续开发，只能进入 Prompt 5 的下载/联系/职聊/面试邀请门禁占位。
