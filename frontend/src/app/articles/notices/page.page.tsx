...
export const metadata: Metadata = {
  title: '网站公告',
  description: 'LocalTalent 网站公告频道展示公开公告和服务通知。'
};
...
export default async function NoticesRoute({ searchParams }: { searchParams?: Promise<SearchParams> }) {
  ...
  try {
    const result = await fetchPortalContents({ ...query, content_type: 'notice' });
    initialState = {
    ...
    initialState = {
      status: 'error',
      query,
      message: error instanceof Error ? error.message : '网站公告暂时不可用。',
    ...
    <PortalShell>
      <ContentChannelPage
        config={{
          ...
          eyebrow: 'Notice',
          title: '网站公告',
          description: '展示平台公开公告和服务提醒，不包含后台备注、审核材料或个人信息。',
